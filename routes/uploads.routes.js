...
router.get('/config', async (_req, res) => {
  try {
    const st = cloudinaryUploads.getCloudinaryConfigStatus();
...
router.get('/ping', async (_req, res) => {
  try {
    ...
    const out = await cloudinaryUploads.cloudinaryPing();
...
async function handleCoverImageUpload(req, res, options = {}) {
  ...
  try {
    ...
    let result = null;
    try {
      result = await cloudinaryUploads.uploadFromBuffer(file.buffer, { folder });
    } catch (streamErr) {
      ...
      try {
        ...
        result = await cloudinaryUploads.uploadFromDataUri(dataUri, { folder });
      } catch (dataErr) {
...
module.exports.handleCoverImageUpload = handleCoverImageUpload;
