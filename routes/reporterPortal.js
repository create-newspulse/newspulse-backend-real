...
function buildReporterAuthTraceId() {
  if (typeof crypto.randomUUID === 'function') return crypto.randomUUID();
  return crypto.randomBytes(12).toString('hex');
}

function ensureReporterAuthTrace(req, res) {
  if (!req?.reporterAuthTraceId) {
    req.reporterAuthTraceId = buildReporterAuthTraceId();
  }
...
function isReporterOtpDevelopmentDelivery() {
...
function shouldExposeDevOtp() {
  ...
  return isReporterOtpDevelopmentDelivery() || String(process.env.OTP_DEV_ECHO || '') === '1';
}
...
function buildOtpRequestSuccessResponse(email, traceId, code, mailResult = {}) {
  const delivery = isReporterOtpDevelopmentDelivery() ? 'development' : (mailResult?.delivery || 'email');
  return buildOtpAcceptedResponse(email, {
    traceId,
    delivery,
    ...(shouldExposeDevOtp() ? { developmentCode: code, devCode: code } : {}),
  });
}
...
function buildOtpLogContext(req, email, extra = {}) {
  ...
  return {
    ...
    requestHostname: getRequestHostname(req) || null,
    requestProtocol: req?.protocol || null,
    secureRequest: !!req?.secure,
    compatRequest: isReporterAuthCompatRequest(req),
...
function getReporterMailerReadiness() {
  ...
  if (isReporterOtpDevelopmentDelivery()) {
    return {
      ...mailerStatus,
      stubMode: true,
      provider: 'stub',
...
function getReporterJwtExpiresIn() {
  return getReporterSessionExpiresIn();
}
...
async function sendReporterOtpEmail(email, code, req) {
  ...
  if (isReporterOtpDevelopmentDelivery()) {
    console.log('[reporter-portal][mail][development-delivery]', buildOtpLogContext(requestLike, email, {
      scope: REPORTER_PORTAL_MAIL_SCOPE,
      provider: 'development',
      sendMethod: 'development-response',
      action: 'send-skipped-development',
      backendCode: null,
    }));
    return { method: 'development-response', delivery: 'development' };
  }

  console.log('[reporter-portal][mail][prepare]', buildOtpLogContext(requestLike, email, {
    scope: REPORTER_PORTAL_MAIL_SCOPE,
    mode: stubMode ? 'stub' : mailerStatus.provider,
    provider: mailerStatus.provider,
    providerOrder: mailerSummary.providerOrder,
    action: 'provider-chosen',
  ...
  if (stubMode) {
    ...
    console.log('[reporter-portal][mail][sent]', buildOtpLogContext(requestLike, email, {
      scope: REPORTER_PORTAL_MAIL_SCOPE,
      provider: 'stub',
      sendMethod: 'stub',
      action: 'send-result',
  ...
  try {
    ...
    console.log('[reporter-portal][mail][transporter-initialized]', buildOtpLogContext(requestLike, email, {
      scope: REPORTER_PORTAL_MAIL_SCOPE,
      provider: mailerStatus.provider,
      providerOrder: mailerSummary.providerOrder,
      hasTransporter: !!transporter,
      action: 'provider-ready',
      secureMode: mailerSummary.secureMode,
    ...
    const failure = resolveReporterMailerFailure(error, mailerStatus);
    console.error('[reporter-portal][mail][transporter-failed]', buildOtpLogContext(requestLike, email, {
  ...
  try {
    ...
    const failure = resolveReporterMailerFailure(error, mailerStatus);
    console.error('[reporter-portal][mail][send-failed]', buildOtpLogContext(requestLike, email, {
      scope: REPORTER_PORTAL_MAIL_SCOPE,
      provider: failure.provider,
      action: 'send-result',
      result: 'failed',
  ...
  if (!accepted.includes(email)) {
    ...
    console.error('[reporter-portal][mail][recipient-rejected]', buildOtpLogContext(requestLike, email, {
      action: 'send-result',
      result: 'recipient-rejected',
      backendCode: 'EMAIL_RECIPIENT_REJECTED',
      accepted,
    }));
    throw createAcceptedOtpRequestError(error);
  }
  ...
  return { method: info?.provider || mailerStatus.provider || 'email', delivery: 'email' };
}
...
router.post('/auth/request-login-otp', requireReporterPortalOpen, async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      ...buildOtpRequestSuccessResponse(email, traceId, code, mailResult),
    });
  } catch (error) {
...
router.post('/auth/verify-login-otp', requireReporterPortalOpen, async (req, res) => {
  try {
    ...
    const reporterSession = await createReporterSession(reporter, { req, now: loginAt });
    const token = reporterSession.token;
    const expiresAt = reporterSession.expiresAt;
...
router.post('/auth/logout', requireReporterPortalOpen, requireReporterPortalAuth, async (req, res) => {
  try {
    ...
    if (isReporterSessionToken(token)) {
      await revokeReporterSessionToken(token, 'logout');
    } else {
...
router.post('/profile/email/confirm-change', requireReporterPortalOpen, requireReporterPortalAuth, async (req, res) => {
  try {
    ...
    await revokeReporterSessionsForReporter(reporterDoc._id, 'email_change');
    await reporterDoc.save();
...
router.get('/auth/session', requireReporterPortalOpen, async (req, res) => {
  const handleVerifiedSession = async () => {
    ...
    const { summary } = buildSummary(submissions);
    const expiresAt = req.reporterPortalSession && req.reporterPortalSession.expiresAt
      ? new Date(req.reporterPortalSession.expiresAt)
      : req.reporterPortalTokenPayload && req.reporterPortalTokenPayload.exp
...
router.post('/submissions', requireReporterPortalOpen, requireReporterPortalAuth, async (req, res) => {
  try {
    ...
    logReporterContactPipeline({
      ...
      reporterAccountId: submission.reporterAccountId ? String(submission.reporterAccountId) : (reporterDoc && reporterDoc._id ? String(reporterDoc._id) : null),
    });
...
router.patch('/submissions/:id', requireReporterPortalOpen, requireReporterPortalAuth, async (req, res) => {
  try {
    ...
    submission.reporterAccountId = submission.reporterAccountId || req.reporterPortal.reporterId;
