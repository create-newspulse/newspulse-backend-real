...
async function getActiveSponsoredFeature(req, res) {
  ...
  const placementRaw = (req.params && req.params.placementKey) || (req.query && req.query.placementKey) || SPONSORED_FEATURE_PLACEMENT_KEYS[0];
  ...
  if (!isDbReady()) {
    return res.status(200).json({ ok: true, placementKey, feature: null });
  }
...
async function getHomepageCenterSlot(req, res) {
  ...
  if (!isDbReady()) {
    return res.status(200).json({
      ...
      selectedSource: 'safe-default',
      selectionOrder: ['sponsored-feature', 'editor-pick', 'top-explainer', 'regional-national-fallback', 'safe-default'],
      item: safeDefaultCard(),
    });
  }

  const activeSponsored = await getActiveSponsoredFeatureByPlacement('HOMEPAGE_SPONSORED_FEATURE', { now });
  if (activeSponsored) {
    return res.status(200).json({
      ok: true,
      slotKey: 'HOMEPAGE_CENTER',
      selectedSource: 'sponsored-feature',
      selectionOrder: ['sponsored-feature', 'editor-pick', 'top-explainer', 'regional-national-fallback', 'safe-default'],
      item: {
        kind: 'sponsoredFeature',
        selectedSource: 'sponsored-feature',
        ...toPublicSponsoredFeatureDto(activeSponsored.feature, activeSponsored.linkedArticle),
      },
    });
  }
