...
const SPONSORED_FEATURE_TYPE = 'sponsored_feature';

function placementKeyToPlacement(value) {
  const key = String(value || '').trim().toUpperCase();
  if (key === 'HOMEPAGE_SPONSORED_FEATURE') return 'homepage_sponsored_feature';
  return null;
}
...
function normalizePlacementKey(value) {
  ...
  if (lower === 'homepage') return 'HOMEPAGE_SPONSORED_FEATURE';
  if (lower === 'homepage_sponsored_feature') return 'HOMEPAGE_SPONSORED_FEATURE';
