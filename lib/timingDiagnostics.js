...
function logPublicNewsCacheDiagnostic(req, event, details = {}, logger = console) {
  try {
    const payload = {
      requestId: getRequestTimingState(req)?.requestId,
      cacheKey: sanitizeDiagnosticString(details.cacheKey, { maxLength: 200 }),
      event: sanitizeDiagnosticString(event),
    };
    for (const field of ['result', 'reason', 'mode']) {
      const value = sanitizeDiagnosticString(details[field]);
      if (value) payload[field] = value;
    }
    for (const field of ['ttlSeconds', 'statusCode']) {
      if (Number.isInteger(details[field]) && details[field] >= 0) payload[field] = details[field];
    }
    if (typeof details.redisReady === 'boolean') payload.redisReady = details.redisReady;
    logger.log('[cache][public-news]', payload);
    return true;
  } catch (_) {
    return false;
  }
