...
function offlineMediaFields(source = {}) {
  return {
    offlinePosterImageUrl: normalizeString(source.offlinePosterImageUrl),
    offlineLoopVideoUrl: normalizeString(source.offlineLoopVideoUrl),
    offlineMessage: normalizeString(source.offlineMessage),
  };
}

function maintenanceCurrentSource(message = 'Live TV video is not available right now.', source = {}) {
  return {
    enabled: false,
...
function stableCurrentSource(source, sourceType, video, overrides = {}) {
  if (!video && sourceType !== 'maintenance') return maintenanceCurrentSource('Live TV video is not available right now.', source);
...
function scheduledCurrentSource(program) {
  ...
  if (!video) return maintenanceCurrentSource('Live TV video is not available right now.', program);
...
function toCurrentSource(liveTv) {
  ...
  if (source.enabled === false) return maintenanceCurrentSource('Live TV video is not available right now.', source);
  ...
  return maintenanceCurrentSource('Live TV video is not available right now.', source);
}
