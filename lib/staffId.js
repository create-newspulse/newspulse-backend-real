...
const RETIRED_STAFF_ID_DELETE_ACTIONS = Object.freeze(['staff_deleted_permanently', 'STAFF_DELETED_PERMANENTLY']);
...
function retireStaffId(staffId) {
  const normalized = normalizeStaffId(staffId);
  if (normalized && !isFounderStaffId(normalized)) retiredStaffIds.add(normalized);
  return normalized;
}

async function isRetiredStaffId(staffId) {
  const normalized = normalizeStaffId(staffId);
  if (!normalized) return false;
  if (retiredStaffIds.has(normalized)) return true;
  if (!hasNativeConnection() || typeof AuditLog.countDocuments !== 'function') return false;

  try {
    const count = await AuditLog.countDocuments({
      action: { $in: RETIRED_STAFF_ID_DELETE_ACTIONS },
      $or: [
        { 'meta.staffId': normalized },
        { 'meta.targetStaffId': normalized },
        { 'meta.deletedStaffId': normalized },
      ],
    });
    if (count > 0) {
      retiredStaffIds.add(normalized);
      return true;
    }
  } catch (_) {}
  return false;
...
async function nextFallbackSequence(year) {
  ...
  while (retiredStaffIds.has(staffIdForSequence(year, sequence))) sequence += 1;
  return sequence;
}
...
async function resolveStaffIdForNewUser(userLike, options = {}) {
  ...
  if (requested) {
    ...
    if (await isRetiredStaffId(requested)) {
      const error = new Error('Staff ID was permanently deleted and cannot be reused');
      error.code = 'STAFF_ID_RETIRED';
      throw error;
    }
