...
const { assertAllowedAdminMediaMimeType, assertAllowedMimeType } = require('./mediaUploadValidation');
...
async function uploadMediaLibraryFile(req, file, options = {}) {
  ...
  const mimeType = Array.isArray(options.allowedMimeTypes)
    ? assertAllowedMimeType(file.mimetype, options.allowedMimeTypes, options.validationMessage)
