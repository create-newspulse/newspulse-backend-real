...
async function deleteAssetByPublicId(publicId, opts = {}) {
  ensureCloudinaryConfigured();
  ...
  const resourceType = String(opts.resourceType || opts.resource_type || 'image').trim() || 'image';
  return await cloudinary.uploader.destroy(id, { resource_type: resourceType, invalidate: true });
}
