...
function currentAccountStatus(user) {
  ...
  if (['suspended', 'locked', 'expired', 'archived', 'deleted', 'deleted_test'].includes(legacy)) return legacy;
  const accountStatus = String(user?.accountStatus || '').toLowerCase();
  if (['active', 'suspended', 'locked', 'expired', 'archived', 'deleted', 'deleted_test'].includes(accountStatus)) return accountStatus;
  return 'active';
}
...
function ensureActiveAccount(user, res) {
  ...
  const code = status === 'suspended' ? 'ACCOUNT_SUSPENDED' : status === 'locked' ? 'ACCOUNT_LOCKED' : status === 'archived' ? 'ACCOUNT_ARCHIVED' : status === 'deleted' || status === 'deleted_test' ? 'ACCOUNT_DELETED' : 'ACCOUNT_EXPIRED';
