...
function buildPublicCategoryFilter(value) {
  ...
  const matchValues = getPublicCategoryMatchValues(key);
  if (!matchValues.length) return null;

  return new RegExp(`^(?:${matchValues.map(escapeRegExp).join('|')})$`, 'i');
}

function getPublicCategoryMatchValues(value) {
  const key = getCanonicalPublicCategoryKey(value);
  if (!key) return [];
  ...
  return Array.from(new Set(matchValues.map((item) => String(item || '').trim().toLowerCase()).filter(Boolean)));
}
