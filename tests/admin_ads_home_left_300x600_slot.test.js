...
test('GET /api/admin/ads lists HOME_LEFT_300x600 ads by exact slot without aliasing', async (t) => {
  stubDbReady(t);
  ...
  Ad.find = (filter) => {
    ...
    return {
      ...
      lean: async () => ([{
        ...
        title: 'Home Left Tower Ad',
        imageUrl: 'https://example.com/home-left-tower.jpg',
        isClickable: true,
  ...
  const res = await request(app)
    .get('/api/admin/ads?slot=HOME_LEFT_300x600')
    .set('Authorization', `Bearer ${makeOpaqueAdminToken()}`);

  assert.equal(res.status, 200);
  assert.deepEqual(capturedFilter, { slot: 'HOME_LEFT_300x600' });
  assert.equal(res.body.ok, true);
  assert.equal(res.body.ads.length, 1);
  assert.equal(res.body.ads[0].slot, 'HOME_LEFT_300x600');
});

test('POST /api/admin/ads creates HOME_LEFT_300x600 as a first-class slot', async (t) => {
  stubDbReady(t);
