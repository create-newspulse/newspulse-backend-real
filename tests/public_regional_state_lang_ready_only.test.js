...
test('GET /api/public/regional supports Gujarat district Vav-Tharad without changing payload shape', async () => {
  const prevFind = Article.find;
  const prevCount = Article.countDocuments;
  ...
  try {
    const dataset = [
      {
        ...
        slug: 'vav-tharad-story',
        slugs: { gu: 'vav-tharad-story' },
        category: 'regional',
        originalLang: 'gu',
        language: 'gu',
        title: 'વાવ-થરાદ સમાચાર',
        summary: 'જિલ્લા અપડેટ',
        content: 'વિગતવાર સામગ્રી',
        coverImage: { url: 'https://img.example/vav-tharad.jpg', publicId: null, alt: null },
        geo: { state: 'gujarat', district: 'vav-tharad', city: null },
        tags: ['state:gujarat', 'district:vav-tharad'],
    ...
    const res = await request(app).get('/api/public/regional?state=gujarat&district=Vav-Tharad&lang=gu&page=1&limit=20');

    assert.equal(res.status, 200);
    assert.equal(res.body.ok, true);
    assert.equal(res.body.success, true);
    assert.equal(res.body.status, 200);
    assert.deepEqual(Object.keys(res.body.data).sort(), ['items', 'lang', 'limit', 'page', 'stateSlug', 'total']);
    assert.equal(res.body.data.stateSlug, 'gujarat');
    assert.equal(res.body.data.lang, 'gu');
    ...
    const item = res.body.data.items[0];
    assert.equal(item.slug, 'vav-tharad-story');
    assert.equal(item.canonicalSlug, 'vav-tharad-story');
    assert.equal(item.stateSlug, 'gujarat');
    assert.equal(item.imageUrl, 'https://img.example/vav-tharad.jpg');
    assert.equal(item.title, 'વાવ-થરાદ સમાચાર');
    assert.equal(item.summary, 'જિલ્લા અપડેટ');
    assert.equal(item.content, 'વિગતવાર સામગ્રી');
    assert.equal(item.provider, 'manual');
    assert.equal(item.generatedAt, null);
    ...
    assert.ok(asJson.includes('geo.state'));
    assert.ok(asJson.includes('geo.district'));
    assert.ok(asJson.includes('vav-tharad'));
    assert.ok(!asJson.includes('vav tharad'));
  } finally {
    Article.find = prevFind;
    Article.countDocuments = prevCount;
